...
10/26
ぐっちーにコード見てもらった。完成！感謝！
Discordに一連の流れあり、復習によい
...
PLAYER_WIDTH = 100 #当たり判定に関わってくる。必ずしも画像の実寸じゃなくてよいようだ
PLAYER_HEIGHT = 150
...
def move_item():
    for i in range(item_num):
        item_y[i] += 1 + i / 3
        if item_y[i] > SURFACE_HEIGHT: #ここを更新しないと画像がウィンドウ下で荒ぶる
            item_y[i] = ITEM_HEIGHT  #ここを更新しないと画像がウィンドウ下で荒ぶる
            item_hit[i] = False
            item_x[i] = random.randint(0, SURFACE_WIDTH)
            for j in range(0, i-1): #アイテム同士を話して落下させたいので変数調整
...
def get_item(category):
    ...
    if (category == "n") or (category == "m"): #このように書かないと村ちゃんの画像が判定されない
        power += 10
        if power > POWER_MAX:
            power = POWER_MAX
    elif (category == "nf") or (category == "mf"): #このように書かないと村ちゃんの画像が判定されない
        power -= 15
        if power < 0:
            power = 0
...
def main():
    ...
    while True:
        ...
        surface.blit(img_bg, [0, 0]) #背景を書いてからその上にゲームタイトルとプレスキー的なやつを書く必要がありますね。
